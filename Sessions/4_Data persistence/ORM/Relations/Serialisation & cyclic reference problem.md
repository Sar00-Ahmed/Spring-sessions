...
> [!Attention]
> Note that this issue is not specific only to JSON serialisation, whenever object need to be serialised to any format so you will need to handle accordingly aand may need different annotations than the one discussed.
