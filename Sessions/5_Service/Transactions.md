Excellent question! `@Transactional` is one of the most important and frequently used annotations in Spring. Here's what you need to know:
...
```java
@SpringBootTest
@Transactional // Test transaction rolls back automatically
class UserServiceTest {
    
    @Test
    @Rollback(true) // Default - test data cleaned up
    void testCreateUser() {
        // Test logic - database changes rolled back after test
    }
    
    @Test
    @Rollback(false) // Commit test data
    void testWithCommit() {
        // Changes persist in database (use carefully!)
    }
...
```java
// Add to application.properties to see transaction logs
...
**Key Takeaway**: `@Transactional` defines the boundaries of a single unit of work. Everything inside executes atomically - all operations succeed or all fail together, maintaining database consistency.